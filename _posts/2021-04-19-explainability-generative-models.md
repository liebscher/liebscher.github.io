...
Natural Language Processing (NLP) models have been gaining popularity like crazy; they're getting thrown into a new industry every week. In the last few years of NLP, we've seen the development of [large language models](https://en.wikipedia.org/wiki/Language_model), which model the statistical properties of language. Generally speaking, there are two main types of language models: discriminative and generative. The former are useful in a wide array of text analytics, from classic sentiment analysis to [detecting misinformation](https://www.technologyreview.com/2021/03/11/1020600/facebook-responsible-ai-misinformation/) on social media platforms. The latter, generative models, have mostly come to light as a result of OpenAI's GPT models. These models are the focus of this post. They're the focus because they're being applied to an exponential number of new problems, and yet experts and researchers are still struggling to explain them.
...
<div class="caption">
  Figure 1. Modern generative language models are developed by setting up a neural network, then training that network to predict the next word in a sequence. After training on billions of words, the model tends to be able to produce fairly comprehensible text. If it produces something wrong though, we have no good way of explaining these mistakes. This is very unlike asking a human to explain what they've said, which usually has satisfactory results.
</div>

Knowing how important it is that progress in this area of NLP be made, I'm laying out this article to discuss: why it matters so much that we be able to explain generative language models, existing work on this question and similar ones, and future directions that we might take. Broadly speaking, one of my interests in life right now is to prevent degenerate NLP models (i.e. models that break down and produce unwelcome content) from inflicting harm on society. That said, let me introduce why explainability matters for these generative models.
...
At the end of the day, engineers, managers, and business leaders will have to answer: Why did the model produce that output? As Silicon Valley (and Austin, NYC, etc.) hurtles toward this precarious scenario, we should be doing our best to anticipate the need for explainability. Plainly said, generative models are becoming ubiquitous, and we can't indulge in them without recognizing their faults. With discriminative models, in NLP but also in computer vision and traditional machine learning, society has been learning the hard way that not all algorithms are objective and interpretable. We can't let this happen again. We can't let the explanation be, "[It's just the algorithm!](https://twitter.com/dhh/status/1192540900393705474?s=20)"
...
Yet, we just don't have anything that comes close to explainability in large generative language models. I hypothesize that part of this is due to the nascency of these models (predecessors were either already fairly explainable or weren't useful enough in practice to demand a need to explain their output), part is due to the sheer volume of data needed to build them, and part is due to how inscrutable these models are, with many having millions or billions of parameters and many layers of abstraction. There are likely other factors playing into their lack of explainability, and as the conversation matures we'll have a better understanding of the obstacles in our way.
...
In fact, there's evidence we do it about once ever minute and a half <d-cite key="dingemanse2015universal"></d-cite>. And although most modern linguists refrain from saying there's anything universal across languages, the conversational repair initiator *Huh?* might in fact be a candidate universal word. This suggests that humans are constantly and consistently asking other humans to clarify or explain what's been uttered <d-cite key="dingemanse2013huh"></d-cite>.

In some cases of miscommunication, the comprehension channel is noisy, or maybe the utterance was just uninterpretable. In other cases, we might ask a person to explain what they've said simply because we want a more detailed response. In any case, we tend to respond with a clarification question, a request for repetition, or some other repair initiator.
...
Lastly, I'd like to reiterate Zachary Lipton's distinction between models transparent to humans and post-hoc explanations. Post-hoc methods for explaining generative language models might accomplish certain goals, but it might be in our favor to continue researching methods which are inherently more understandable to humans. The latter is crucial for public science communication, but both will probably be an aspect of future algorithmic recourse.
...
Unfortunately, this article is a bit longer and a bit less accessible than I originally intended. I found it incredibly difficult to distill the most important aspects of this problem